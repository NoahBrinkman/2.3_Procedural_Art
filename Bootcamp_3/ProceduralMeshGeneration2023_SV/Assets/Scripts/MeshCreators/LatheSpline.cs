...
public class LatheSpline : MeshCreator {
	...
	public override void RecalculateMesh() {
		...
		float minY = _vertices.OrderBy(v => v.y).ToArray()[0].y;
		float maxY = _vertices.OrderByDescending(v => v.y).ToArray()[0].y;
		...
		for (int curveIndex = 0; curveIndex <= curveCount; curveIndex++) {
			...
			for (int vertexIndex = 0; vertexIndex<vertexCount; vertexIndex++) {
				...
				//y - min / diff
				uvXOffset += 1.0f / (float)NumCurves;
				Vector2 uv = new Vector2(uvXOffset, (float)vertex.y - minY / (maxY-minY));
				uv *= UVScale;
