...
public class AutoUv : MonoBehaviour
{
	...
	public bool meshHasChanged = false;
    void Update()
    {
        if ((meshHasChanged && UseWorldCoordinates && AutoUpdate) || Input.GetKeyDown(KeyCode.F2)) {
			UpdateUvs();
	...
	void ComputeTangents(Vector3 v1, Vector3 v2, Vector3 v3, out Vector3 tangent, out Vector3 biTangent) {
		...
		Vector3 a = (v3 - v1).normalized;
		Vector3 b = (v2 - v1).normalized;
		...
		Vector3 up = Mathf.Abs(normal.y) > 0.98 ? Vector3.right : Vector3.up;
