...
public class WarpMeshAlongSpline : MeshCreator
{
	...
	public override void RecalculateMesh() {
		...
		for (int i = 0; i < points.Count-1; i++) {
			...
			Vector3 lineSegmentDirection = new Vector3();
			if (i <= 0)
			{
				 lineSegmentDirection = (points[i+1]-points[i]).normalized;
			}

			else
			{
				lineSegmentDirection = (points[i+1]-points[i] + points[i]-points[i-1]).normalized;
			}
			
			//points[i-1]+points[i]/2.normalized;
		...
		for (int i = 0; i < points.Count-2; i++) {
			...
			for (int j = 0; j<InputMesh.vertexCount; j++) {
				...
				Vector3 rotatedXYModelCoordinate;
				Quaternion interpolatedQuaternion = Quaternion.Lerp(localOrientation[i],localOrientation[i+1],t);
				rotatedXYModelCoordinate = interpolatedQuaternion * inputV;
					builder.AddVertex(
			...
			int numSubMesh = InputMesh.subMeshCount;
			Debug.Log("Submesh count: " + numSubMesh);
			for (int k = 0; k < numSubMesh; k++)
			{
				int[] numTris = InputMesh.GetTriangles(k);
				for (int l = 0; l < numTris.Length; l+=3) {
					
					builder.AddTriangle(
						numTris[l] + numVerts * i,
						numTris[l+1] + numVerts * i,
						numTris[l+2] + numVerts * i,
						k
					);
				}
